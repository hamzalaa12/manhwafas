...
class AutoMangaServiceIntegrated {
  ...
  async syncAllSources(): Promise<SyncResult> {
    ...
    this.isRunning = true;
    await loggingService.info('بدء مزامنة جميع المصادر', {}, 'sync');
    ...
    try {
      for (const [sourceId, source] of this.sources) {
        ...
        // انتظار بين المصادر لتجنب إرهاق الخوادم
  ...
  private async syncSource(source: MangaSource): Promise<SyncResult> {
    ...
    try {
      ...
      for (const mangaData of mangaList) {
        try {
          const processResult = await this.processMangaData(mangaData, source);
          
          if (processResult.isNew) {
            result.newManga++;
            await loggingService.logMangaAdded(mangaData.title, source.id);
          ...
          await loggingService.error(`خطأ في معالجة المانجا: ${mangaData.title}`, error, 'manga', source.id);
        }
      }
  ...
  private async processMangaData(mangaData: MangaData, source: MangaSource) {
    ...
    try {
      ...
      if (duplicateCheck.isDuplicate) {
        ...
        // فحص الفصول الجديدة للمانجا الموجودة
        if (duplicateCheck.matchedManga && mangaData.chapters) {
          result.newChapters = await this.syncNewChapters(duplicateCheck.matchedManga.id, mangaData.chapters);
        }
      ...
      // إضافة مانجا جديدة
      const mangaId = await this.addNewManga(mangaData);
      result.isNew = true;

      if (mangaData.chapters && mangaData.chapters.length > 0) {
        result.newChapters = mangaData.chapters.length;
        await this.addChapters(mangaId, mangaData.chapters);
      }
  ...
  @trackPerformance
  private async addNewManga(mangaData: MangaData): Promise<string> {
  ...
  @trackPerformance
  private async syncNewChapters(mangaId: string, chapters: ChapterData[]): Promise<number> {
    ...
    for (const chapterData of chapters) {
      ...
      if (!duplicateCheck.isDuplicate) {
        ...
        await loggingService.logChapterAdded(
          `مانجا ${mangaId}`,
  ...
  async deleteSource(id: string) {
    try {
      ...
      await loggingService.error(`فشل في حذف المصدر: ${id}`, error, 'sources');
      throw error;
    }
  }
...
export const autoMangaServiceIntegrated = new AutoMangaServiceIntegrated();
